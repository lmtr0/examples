...
#[cfg(unix)]
use tokio::{sync::mpsc, net::UnixListener};
...
impl pb::echo_server::Echo for EchoServer {
    async fn echo(&self, _: Request<EchoRequest>) -> EchoResult<EchoResponse> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let incoming = {
        let uds = UnixListener::bind("lock.sock")?;
    ...
    Server::builder()
        .add_service(pb::echo_server::EchoServer::with_interceptor(server, check_auth))
        .serve_with_incoming(incoming)
        .await?;

    
    std::fs::remove_file("lock.sock")?;
    Ok(())
}
